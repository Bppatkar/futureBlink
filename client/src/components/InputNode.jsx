...
export function InputNode({ data }) {
  ...
  return (
    <div
      className={`min-w-125 rounded-2xl p-6 shadow-2xl transition-all-300 ${
        data.darkMode
          ? 'bg-linear-to-br from-gray-900 to-gray-800'
          : 'bg-linear-to-br from-white to-gray-50'
      } ${isFocused ? 'scale-102' : ''} border ${
    ...
    >
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm opacity-70 mt-1">
              Ask anything about AI, technology, or coding
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            className={`p-2 rounded-lg transition-all-300 ${
              data.darkMode ? 'hover:bg-white/10' : 'hover:bg-gray-100'
            }`}
            title="Voice Input"
          >
      ...
      <div className="relative mb-4">
        <textarea
          ...
          className={`w-full p-4 rounded-xl border transition-all-300 resize-none min-h-45 focus:outline-none focus:ring-2 focus:ring-blue-500/50 ${
            data.darkMode
          ...
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-medium transition-all-300 ${
              localPrompt.trim() && !data.isLoading
                ? 'bg-linear-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white shadow-lg hover:shadow-xl hover:scale-105'
                : data.darkMode
      ...
      <div className="mt-4 pt-4 border-t border-white/10">
        ...
        <div className="flex flex-wrap gap-2">
          {['Future of AI', 'Code review', 'Tech trends', 'Explain ML'].map(
            (prompt) => (
              <button
                key={prompt}
                onClick={() => {
